...
class Network(nn.Module):
    def __init__(self, cfg, device):
        ...
        regularizer.extend([SparseCostReg(self.group_channels[i]) for i in range(1,self.resolution_levels)])
    ...
    def forward(self, data):
        ...
        for level in range(self.resolution_levels):
            ...
            if level == 0:
                ...
                vwa_net = None
                hypos[level], hypo_coords[level], intervals[level] = top_k_hypothesis_selection(
                                                        prob_grids[level-1],
            ...
            #### Cost Regularization ####
            occ_grid = self.regularizer[level](cost_volume, hypo_coords[level], mode=self.mode)
        ...
        prob_volume_sum4 = 4 * F.avg_pool3d(F.pad(prob_grids[0], pad=(0, 0, 0, 0, 1, 2)), (4, 1, 1), stride=1, padding=0).squeeze(1)
        max_sum4_prob, _ = prob_volume_sum4.max(dim=1)
        confidence = F.interpolate(max_sum4_prob.unsqueeze(1),scale_factor=8,mode='bilinear',align_corners=False)
