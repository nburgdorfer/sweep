...
def get_argparser():
    ...
    parser.add_argument(
        "--log_path", type=str, help="Path to log directory.", required=True
    )
    parser.add_argument(
        "--dataset",
        type=str,
        help="Current dataset being used.",
        required=True,
        choices=["ScanNet", "Replica", "DTU", "TNT"],
    )
    parser.add_argument(
        "--model",
        type=str,
        ...
        required=True,
        choices=["NP_CVP_MVSNet", "GBiNet", "MVSNet"],
    )
    return parser
