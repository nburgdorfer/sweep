...
def projection_eval(est_depth_path, est_conf_path, dataset):
    ...
    depth_files = np.stack(
        (est_depth_files, filt_depth_files, proj_depth_files), axis=-1
    )
    ...
    for i, (edf, fdf, pdf) in enumerate(depth_files):
        ...
        perc_better[i] = better_inds.shape[0] / (
            better_inds.shape[0] + worse_inds.shape[0]
        )
...
def depth_fscore(est_depth_path, est_conf_path, dataset, th=2.0):
    ...
    for i, (edf, fdf, pdf) in enumerate(depth_files):
        ...
        precision[i, 0] = np.argwhere(dense_error <= th).shape[0] / (
            dense_error.shape[0] + 1e-10
        )
        recall[i, 0] = np.argwhere(dense_error <= th).shape[0] / (
            dense_error.shape[0] + 1e-10
        )
        fscore[i, 0] = (2 * precision[i, 0] * recall[i, 0]) / (
        ...
        precision[i, 1] = np.argwhere(filt_acc <= th).shape[0] / (
            filt_acc.shape[0] + 1e-10
        )

        filt_comp = filt_error * gt_mask
        filt_comp = filt_comp[gt_mask > 0]
        recall[i, 1] = np.argwhere(filt_comp <= th).shape[0] / (
            filt_comp.shape[0] + 1e-10
        )
        fscore[i, 1] = (2 * precision[i, 1] * recall[i, 1]) / (
        ...
        precision[i, 2] = np.argwhere(proj_acc <= th).shape[0] / (
            proj_acc.shape[0] + 1e-10
        )

        proj_comp = proj_error * gt_mask
        proj_comp = proj_comp[gt_mask > 0]
        recall[i, 2] = np.argwhere(proj_comp <= th).shape[0] / (
            proj_comp.shape[0] + 1e-10
        )
        fscore[i, 2] = (2 * precision[i, 2] * recall[i, 2]) / (
            precision[i, 2] + recall[i, 2]
        )
...
def eval_2d(paths, dataset, scene, vis_path=None):
    ...
    for i, edf in enumerate(est_depth_files):
        ...
        percs = depth_percentages(
            maps["est_depth"], maps["target_depth"], thresholds=thresholds, mask=mask
        )
        percentages[i] = percs
    ...
    print(
        f"Percentages: {percentages[0]:3.2f}%  |  {percentages[1]:3.2f}%  |  {percentages[2]:3.2f}%  |  {percentages[3]:3.2f}%"
    )
...
def target_coverage(data, outputs):
    ...
    loss = []
    near_depth = (
        torch.ones((cfg["training"]["batch_size"])).to(cfg["device"])
    ...
    valid_gt_mask = (
        -F.max_pool2d(-target_depth.unsqueeze(1), kernel_size=5, stride=1, padding=2)
    ...
    for level in reversed(range(len(cfg["model"]["gwc_groups"]))):
        if level == 0:
            ...
            mask = (
                -F.max_pool2d(
                    -target_depth.unsqueeze(1), kernel_size=5, stride=1, padding=2
                )
            ) > near_depth[:, None, None, None]
            tmp_loss = F.smooth_l1_loss(
                regressed_depth[mask], gt_depth[mask], reduction="none"
            )

            tmp_high_frequency_mask = high_frequency_mask[mask]
            tmp_high_frequency_weight = tmp_high_frequency_mask.float().mean()
            weight = (1 - tmp_high_frequency_weight) * tmp_high_frequency_mask + (
                tmp_high_frequency_weight
            ) * (~tmp_high_frequency_mask)
            tmp_loss *= weight
            tmp_loss *= 0.1
        ...
        unfolded_patch_depth = F.unfold(
            target_depth.unsqueeze(1),
            unfold_kernel_size,
            dilation=1,
            padding=0,
            stride=unfold_kernel_size,
        )
        unfolded_patch_depth = unfolded_patch_depth.reshape(
            B, 1, unfold_kernel_size**2, H, W
        )
        # valid gt depth mask
        ...
        occ_hypos_count = (
            (gt_occ_grid > 0).sum(dim=2, keepdim=True).repeat(1, 1, D, 1, 1)
        )
