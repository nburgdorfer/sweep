...
def run_tsdf_fusion(cfg, save_path):
    ...
    for i, frame in enumerate(tqdm(dataset)):
        ...
        rgbd = o3d.geometry.RGBDImage.create_from_color_and_depth(
            rgb, depth, depth_scale=1.0, depth_trunc=8.0, convert_rgb_to_intensity=False
        )
        # to OpenCV
        c2w[:3, 1] *= -1
        c2w[:3, 2] *= -1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="./configs/Replica/office0.yaml",
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        help="Path to dataset sequence. This has higher priority.",
        default="./data/Replica/office0",
    )
    ...
    init_param.intrinsic.set_intrinsics(
        init_param.intrinsic.width, init_param.intrinsic.height, fx, fy, cx, cy
    )
