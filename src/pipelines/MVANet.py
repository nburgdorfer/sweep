...
class Pipeline(BasePipeline):
    ...
    def set_reference_view(self, images, extrinsics, target_depths, reference_index):
        assert reference_index >= 0 and reference_index < images.shape[1]

        if reference_index == 0:
            return (images, extrinsics, target_depths)
        ...
        swap_indices = torch.tensor(swap_indices, dtype=torch.int64, device=images.device)
        
        return (
            images[:, swap_indices],
            extrinsics[:, swap_indices],
            target_depths[:, swap_indices],
        )
    ...
    def run(self, mode, epoch):
        ...
        with tqdm(
            ...
            for batch_ind, data in enumerate(loader):
                ...
                extrinsics = torch.clone(data["extrinsics"])
                all_target_depths = torch.clone(data["all_target_depths"])
                    ...
                    if i != 0:
                        (data["images"], data["extrinsics"], data["all_target_depths"]) = (
                            self.set_reference_view(
                                images, extrinsics, all_target_depths, i
                            )
                        )
