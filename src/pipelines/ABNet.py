...
# GBiNet Network
from src.networks.ABNet import Network
...
class Pipeline(BasePipeline):
    ...
    def compute_loss(self, data, output, resolution_stage):
        loss = {}
        target_depth = data["target_depths"][resolution_stage]

        ### CE-Loss
        # target_labels, mask = build_labels(target_depth, output["max_hypotheses"])
        ...
        loss["total"] = rmse * self.loss_weights[resolution_stage]
    ...
    def run(self, mode, epoch):
        ...
        with tqdm(
            ...
            for batch_ind, data in enumerate(loader):
                ...
                data["selected_plane"] = [None] * (len(self.resolution_stages))
                ...
                for iteration, resolution_stage in enumerate(self.resolution_stages):
                    ...
                    if iteration < num_stages - 1:
                        data["selected_plane"][iteration + 1] = output["selected_plane"]
