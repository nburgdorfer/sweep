...
class BasePipeline:
    def __init__(
        ...
        if self.mode == "inference":
            ...
            self.data_path = os.path.join(
                self.cfg["data_path"], self.inference_scene[0]
            )
            self.output_path = os.path.join(
                self.cfg["output_path"], self.inference_scene[0]
            )
    ...
    def save_output(self, data, output, sample_ind):
        with torch.set_grad_enabled(
            ...
            conf_filename = os.path.join(
                self.conf_path, "disp", f"{sample_ind:08d}.png"
            )
            ...
            depth_map = (
                output["final_depth"][0, 0].detach().cpu().numpy()
    ...
    def build_dataset(self):
        if self.mode == "training":
            self.training_dataset = build_dataset(
                self.cfg, "training", self.training_scenes
            )
            self.cfg["H"], self.cfg["W"] = (
                self.training_dataset.H,
                self.training_dataset.W,
            )
            ...
            self.validation_dataset = build_dataset(
                self.cfg, "validation", self.validation_scenes
            )
            ...
            self.inference_dataset = build_dataset(
                self.cfg, self.mode, self.inference_scene
            )
            self.cfg["H"], self.cfg["W"] = (
                self.inference_dataset.H,
                self.inference_dataset.W,
            )
    ...
    def build_model(self):
        ...
        if self.mode == "inference":
            ...
            if self.cfg["training"]["ckpt_file"] != None:
                print(
                    f"Continuing training pretrained model '{self.cfg['training']['ckpt_file']}'."
                )
    ...
    def build_scheduler(self):
        ...
        self.lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer=self.optimizer, milestones=[10, 12, 14], gamma=0.5
        )
        # ### CosineAnnealingLR
