...
class Pipeline(BasePipeline):
    ...
    def compute_loss(self, data, output):
        ...
        near_depth = (
            torch.ones((batch_size)).to(target_depth) * self.cfg["camera"]["near"]
        )
        far_depth = (
            torch.ones((batch_size)).to(target_depth) * self.cfg["camera"]["far"]
        )
        ...
        for level in range(resolution_levels):
            if level == resolution_levels - 1:
                ...
                mask = (
                    -F.max_pool2d(-target_depth, kernel_size=5, stride=1, padding=2)
                ...
                tmp_loss = F.smooth_l1_loss(
                    final_depth[mask], target_depth[mask], reduction="none"
                )
                ...
                # valid gt depth mask
                mask = (unfolded_patch_depth > near_depth.view((B, 1, 1, 1, 1))).all(
                    dim=2
                )
                mask *= (unfolded_patch_depth < far_depth.view((B, 1, 1, 1, 1))).all(
                    dim=2
                )
    ...
    def compute_stats(self, data, output):
        with torch.set_grad_enabled(
            ...
            mae, acc = depth_acc(
                output["final_depth"][0, 0], data["target_depth"][0, 0]
            )
    ...
    def training(self, epoch):
        ...
        with tqdm(
            data_loader, desc=f"NP-CVP-MVSNet - {mode}", unit="batch"
        ) as loader:
    ...
    def inference(self):
        ...
        with tqdm(
            ...
            for batch_ind, data in enumerate(loader):
                ...
                ## Visualization
                if self.cfg["inference"]["visualize"] and batch_ind % self.cfg["inference"]["vis_freq"] == 0:
                    visualize_mvs(
                        data,
                        output,
                        batch_ind,
