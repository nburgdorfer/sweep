...
from src.components.layers import Conv3d, ConvTranspose3d
...
class CostRegNet(nn.Module):
    def __init__(self, in_channels, c, out_channels=1):
        ...
        self.deconv7 = ConvTranspose3d(
            c * 8, c * 4, stride=(1, 2, 2), padding=1, output_padding=(0, 1, 1)
        )
        self.deconv8 = ConvTranspose3d(
            c * 4, c * 2, stride=(1, 2, 2), padding=1, output_padding=(0, 1, 1)
        )
        self.deconv9 = ConvTranspose3d(
            c * 2, c * 1, stride=(1, 2, 2), padding=1, output_padding=(0, 1, 1)
        )
...
class DenseCostReg(nn.Module):
    def __init__(self, feature_ch):
        ...
        self.conv2a = Conv3d(
            base_ch * 2, base_ch * 4, stride=2, kernel_size=3, padding=1
        )
        ...
        self.conv3a = Conv3d(
            base_ch * 4, base_ch * 8, stride=2, kernel_size=3, padding=1
        )
...
#             SparseConv3d(
#                 base_ch, base_ch * 2, stride=2, kernel_size=2, factorize=False
#             ),
...
#             SparseConv3d(
#                 base_ch * 2, base_ch * 4, stride=2, kernel_size=2, factorize=False
#             ),
...
#             SparseConv3d(
#                 base_ch * 4, base_ch * 8, stride=2, kernel_size=2, factorize=False
#             ),
...
#         coords_b, plain_coords_z, coords_h, coords_w = torch.where(
#             torch.ones_like(hypo_coords.squeeze(1))
#         )
