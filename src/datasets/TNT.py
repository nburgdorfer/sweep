...
class TNT(BaseDataset):
    ...
    def build_samples(self):
        ...
        for scene in scenes:
            if not os.path.isdir(os.path.join(self.data_path, scene, "Images")):
                print(
            ...
            if self.sample_mode == "stream":
                self.total_samples.extend(
                    self.build_samples_stream(scene, curr_frame_count)
                )
    ...
    def build_samples_stream(self, scene, frame_count):
        ...
        for ref_frame in range(frame_count):
            ...
            for ind in frame_inds:
                image_files.append(
                    os.path.join(self.data_path, scene, "Images", f"{ind:08d}.png")
                )
                depth_files.append(
                    os.path.join(
                        self.data_path, scene, "Depths", f"{ind:08d}_depth.pfm"
                    )
                )
                pose_files.append(
                    os.path.join(self.data_path, scene, "Cameras", f"{ind:08d}_cam.txt")
                )
    ...
    def get_cluster_file(self, scene):
        return os.path.join(self.data_path, scene, "Cameras/pair.txt")
    ...
    def load_intrinsics(self, scene):
        cam_file = os.path.join(self.data_path, scene, "Cameras/00000000_cam.txt")
    ...
    def get_all_poses(self, scene):
        poses = []
        pose_path = os.path.join(self.data_path, scene, "Cameras")
